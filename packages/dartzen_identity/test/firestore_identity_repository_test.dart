...
void main() {
  group('FirestoreIdentityRepository (integration w/ mocked FirestoreRestClient)', () {
    ...
    test('verifyEmail activates pending identity and patches document', () async {
    ...
    test(
      'getIdentityById returns unknown error when getDocument fails',
      () async {
    ...
    test('changeRoles with capabilities only patches successfully', () async {
      ...
      const repo = FirestoreIdentityRepository();
      final authority = Authority(
        capabilities: {const Capability.reconstruct('can_view')},
      );
      final result = await repo.changeRoles(id, authority);
      expect(result.isSuccess, isTrue);
    });
    ...
    test(
      'verifyEmail returns unauthorized when activation invalid (revoked)',
      () async {
