...
class DartzenDemoServer {
  /// Creates the server.
  DartzenDemoServer({required this.port, required this.storageBucket});
  ...
  Future<void> initialize() async {
    ...
    _storageReader = FirebaseStorageReader(
      config: FirebaseStorageConfig(bucket: storageBucket),
    );
  ...
  Future<void> run() async {
    ...
    router.get(
      ...
      (Request request, String userId) async => await _withAuth(
        request,
        (identity) => _handleProfile(request, userId, identity),
      ),
    );
  ...
  Future<Response> _withAuth(
    ...
    if (authHeader == null || !authHeader.startsWith('Bearer ')) {
      return Response(
        401,
        body: jsonEncode({'error': AuthError.missingAuthHeader.code}),
    ...
    if (!verifyResult.isSuccess) {
      ...
      return Response(
        401,
        body: jsonEncode({'error': AuthError.invalidToken.code}),
        headers: {'Content-Type': 'application/json'},
      );
    }
    ...
    if (!idResult.isSuccess) {
      ...
      return Response(
        400,
        body: jsonEncode({'error': 'invalid_user_id'}),
        headers: {'Content-Type': 'application/json'},
      );
    }
    ...
    final identityResult = await _identityService.getOrCreateDemoIdentity(
      identityId,
    );
    if (!identityResult.isSuccess) {
      return Response(
        500,
        body: jsonEncode({'error': IdentityError.createFailed.code}),
        headers: {'Content-Type': 'application/json'},
      );
    }
  ...
  Future<Response> _handleLogin(Request request) async {
    try {
      ...
      if (email == null || password == null) {
        return Response(
          400,
          body: jsonEncode({'error': AuthError.invalidCredentials.code}),
          headers: {'Content-Type': 'application/json'},
        );
      }
      ...
      return Response.ok(
        jsonEncode({'idToken': idToken, 'userId': userId}),
        headers: {'Content-Type': 'application/json'},
      );
    } catch (e, stack) {
      ...
      return Response(
        500,
        body: jsonEncode({'error': 'login_failed'}),
        headers: {'Content-Type': 'application/json'},
      );
    }
  }
  ...
  Future<Response> _handleTerms(Request request) async {
    try {
      ...
      return Response.internalServerError(
        body: jsonEncode({'error': TermsError.loadFailed.code}),
        headers: {'Content-Type': 'application/json'},
      );
    }
  }
    ...
    channel.stream.listen(
      (dynamic message) {
        try {
          ...
          final incomingMessage = contracts.WebSocketMessageContract.fromJson(
            json,
          );
