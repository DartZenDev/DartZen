...
The DartZen server is a **GCP-native runtime** built on top of **Shelf**, designed for clarity, performance, explicit behavior, and deterministic execution.


It is not a framework, not an abstraction layer over cloud services, and not a place for domain logic. It is an **execution environment** that wires together infrastructure, configuration, and lifecycle in a predictable and inspectable way.
...
The DartZen server runtime enforces a strict execution model where **HTTP request handling is never blocked by CPU-heavy or long-running operations**.

Unlike traditional event-loop-based servers (including most Shelf-based setups), DartZen treats execution as a first-class runtime concern:

- HTTP transport is kept lightweight and responsive
- CPU-bound and long-running work is executed outside the request loop
- Concurrency is isolate-based, explicit, and deterministic
- No shared mutable state between execution contexts

This model is **enforced by design**, not by convention.

Application code cannot accidentally block the server.
Execution boundaries are explicit, inspectable, and predictable under load.

### 3. GCP-Native by Design
...
Jobs are not a special case or a different architecture. They use the same runtime principles, configuration, and infrastructure wiring.
